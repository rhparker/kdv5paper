...
\end{equation}
By the expansion \eqref{conjlemmaP}, $V^+(x; \lambda) = e^{\nu(\lambda)x}( V_0(\lambda) + V_1^+(x; \lambda))$, where we define $V_1^+(x; \lambda) = \Theta_+(x; \lambda) V_0(\lambda)$
Similarly, we define 
\begin{align*}
...
M^c = \int_{-\infty}^\infty q(y) v^c(y) dy,
\end{align*}
$q(y)$ is the first component of $Q(y)$, and $v^c(y)$ the first component of $V^c(y)$.
\begin{proof}
...
\begin{align*}
\tilde{\Phi}_+^{s/u/c}(x, y; 0) &= P^-(y; 0) \Phi(y, x; 0) P^{s/u/c}(0) P_-(x; \lambda)^{-1} && x, y \leq 0.
\end{align*}
Since the center subspace is one-dimensional, we have the formula
\begin{equation*}
\tilde{\Phi}_+^c(x, y; 0) = \langle W_0, \cdot \rangle V^c(x)
\end{equation*}
Using these with equation \cref{tildeVsolves} we can formally write $Y(x)$ in integrated form as
...
\]
where $v^c(x)$ is the first component of $V^c(y)$. Since $|V^c(y) - V_0|\leq C e^{-\alpha y}$ by \ref{lemma:Vcexists} and the first component of $V_0$ is $1/c$, 
\begin{align*}
...
\end{align*}
since $q(y)$ is the last component of $\Psi(y)$ and both $v^c(y)$ and $q(y)$ are even functions. Expanding $V^+(0; \lambda)$ in a Taylor series about $\lambda = 0$ and using equations \cref{VtildeVderiv}, \cref{W0IPwithtildeV}, and $\langle \Psi(0), V^c(0)\rangle = 0$, we obtain the second equation in \cref{VpmIPs}.
\end{proof}
\end{lemma}
