...
This paper is organized as follows. In \cref{sec:setup}, we set up the problem of interest in general terms, for which KdV5 is a special case. We then present the main results of this paper, which concern the existence (\cref{sec:perexist}) and spectrum (\cref{sec:perstab}) of periodic multi-pulse solutions. This is applied to the periodic single pulse in \cref{sec:persingle} and the periodic double pulse in \cref{sec:perdouble}. In \cref{sec:numerics}, we present numerical results which provide verification for our theoretical work. Our results are then summarized in \cref{sec:conclusions}, and some possible directions for future work are offered. All proofs are deferred to appendices.
...
In this section, we prove the existence of periodic multi-pulse solutions to \cref{genODE}. Heuristically, we construct a periodic multi-pulse by gluing together multiple copies of the primary pulse end-to-end in a loop.
\begin{figure}[H]
\begin{center}
...
Assume Hypotheses \ref{Ehyp}, \ref{Hhyp}, \ref{hypeqhyp}, \ref{Qexistshyp}, and \ref{H0transversehyp}, and \ref{Qclocalhyp}. Let $Q(x)$ be the primary pulse solution, and let $\Psi(x)$ be the solution to the adjoint variational equation from Lemma \ref{varadjsolutions}. Choose any periodic parameterization $(m_0, \dots, m_{n-1}, \theta)$ with $\theta \notin \{-\pi + p^*, p^* \}$, and let $r_*$ be as in \cref{perexist}. 
...
\section{Spectrum of periodic single pulse}\label{sec:persingle}
...
\section{Spectrum of periodic 2-pulse}\label{sec:perdouble}
...
\begin{align}\label{S1for2pulse}
S&_1(\lambda) = \\
&\begin{pmatrix}
e^{-\nu(\lambda)X_1} & -e^{\nu(\lambda)X_0} & \lambda(q(X_0) - q(X_1)) -\tilde{M}^c \lambda^2 & -\lambda(q(X_0) - q(X_1)) \\
-e^{\nu(\lambda)X_1} & e^{-\nu(\lambda)X_0} & \lambda(q(X_0) - q(X_1)) & -\lambda(q(X_0) - q(X_1)) -\tilde{M}^c \lambda^2 \\
