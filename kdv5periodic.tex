...
\end{aligned}
\end{equation}
Let $\alpha_0, \beta_0$ be defined as in \cref{hyp:hypeq}. Choose any $\eta > 0$ with $4 \eta < \alpha_0$, and let $\alpha = \alpha_0 - 2 \eta$. Choose $\delta > 0$ sufficiently small so that for all $|\lambda| < \delta$,
\begin{enumerate}[(i)]
...
|e^{\nu(\lambda)X_i}| \leq C e^{\eta X^*},
\end{align}
where $X^*$ is defined in \cref{Xstar}. The result of \cref{blockmatrixtheorem} will hold for $|\lambda| < \delta$, and as we proceed, we may need to decrease $\delta$.
...
\end{equation}
For all $\lambda$, $A(Q(x); \lambda)$ decays exponentially to the constant-coefficient matrix $A(\lambda)$, i.e. 
\[
...
\end{align}
The equation $Z'(x) = A(\lambda) Z(x)$ has a solution $Z(x) = V_0(\lambda)e^{\nu(\lambda)x}$ which lies in the eigenspace spanned by $V_0(\lambda)$. In the next lemma, we show that equation \cref{vareqlambda} has solutions $V^\pm(x; \lambda)$ on $\R^+$ and $\R^-$ which approach $V_0(\lambda)e^{\nu(\lambda)x}$ as $x \rightarrow \pm \infty$. 
...
\end{cases}
\]
and $V^c(-x) = R V^c(x)$. In addition, we note that by reversibility and \cref{AQsymmetry}, $Y^- = R Y^+$, thus $V^c(0)$ can contain no component in $Y^+ \oplus Y^-$, since $Y^+$ and $Y^-$ only have trivial intersection. The next lemma evaluates two important inner products involving $V^\pm(0; \lambda)$.
