...
We have the following theorem concerning the existence of localized solutions to \cref{KdV5eq4}, which is a direct consequence of \cite{Groves1998} and reversibility.
...
\end{equation}
which can be checked numerically. To examine the stability of a multi-pulse solution $q_n$, we first study the spectrum of the linear operator $\partial_x \calE''(q_n)$, which is the linearization of the PDE \eqref{KdV5c} about $q_n$. It follows from the Weyl essential spectrum theorem \cite[Theorem 2.2.6]{Kapitula2013} and \cite[Theorem 3.1.11]{Kapitula2013} that the essential spectrum of $\partial_x \calE''(u^*)$ is the entire imaginary axis, which depends only on the rest state and is independent of $u^*$. Spectral stability thus depends on the point spectrum, which must come in quartets $\{ \pm \lambda, \pm \overline{\lambda}\}$ since \cref{KdV5c} is Hamiltonian \cite[Proposition 5.1.2]{Kapitula2013}. Since $\partial_x \calE''(q_n) (\partial_x q_n) = 0$ and $\partial_x \calE''(q_n) (-\partial_c q_n) = \partial_x q_n$, the kernel of $\partial_x \calE''(q_n)$ has algebraic multiplicity at least 2 and geometric multiplicity at least 1. For the primary pulse solution $q(x)$, these are the only eigenvalues. For an $n$-pulse solution, there will be $n-1$ additional sets of eigenvalues near 0 \cite{Sandstede1998}. Since these result from nonlinear interactions between the tails of neighboring pulses, we will refer to these as interaction eigenvalues. For double pulses, there is a pair of interaction eigenvalues which is either real or imaginary (to leading order) with negative Krein signature \cite[Theorem 2.3]{Pelinovsky2007}. Using an exponentially weighted space, we can extend this result to arbitrary multi-pulses through a suitable adaptation of \cite{Sandstede1998}. Any potentially imaginary eigenvalues, however, can only be located to leading order. Since the system is no longer Hamiltonian when posed in an exponentially weighted space, we cannot conclude that these eigenvalues are in fact on the imaginary axis.

The presence of the essential spectrum along the entire imaginary axis makes any purely imaginary extremely difficult to locate. As an alternative, we will look at multi-pulses on a periodic domain. From a spatial dynamics perspective, a periodic multi-pulse is a multi-loop periodic orbit which remains close to the primary homoclinic orbit. When posed on a periodic domain, the essential spectrum of $\partial_x \calE''(u^*)$ becomes a discrete set of eigenvalues located on the imaginary axis. For simplicity, we will refer to these as essential spectrum eigenvalues, although they are elements of the point spectrum. As in \cite{Sandstede1998}, we reduce the computation of the spectrum of $\partial_x \calE''(u^*)$ in a ball around the origin to computing the determinant of a matrix $S(\lambda)$. In the periodic case, $S(\lambda)$ is a $2n \times 2n$ matrix, and solving $\det S(\lambda) = 0$ will give us both the interaction eigenvalues and the essential spectrum eigenvalues.
...
\noi \cref{hyp:E}(ii) is reversibility, and \cref{hyp:E}(iii) is translation invariance. \cref{hyp:E}(iv) holds in applications such as KdV5 and lets us write $\partial_x \calE'(u) = 0$ as a first order system. Differentiating the reversibility relation $\calE(u) = \calE(\rho(u))$ with respect to $u$,
\[
...
\noi It follows from \cref{hyp:H} that $H$ is conserved along solutions $U(x)$ to \cref{genODE}. Since $F(0; c) = 0$ for all $c$, the rest state $U = 0$ is an equilibrium of \cref{genODE} for all $c$. The next hypothesis addresses the hyperbolicity of this equilibrium. Although the eigenvalue pattern described in \cref{hyp:hypeq} is not necessary for the existence of a homoclinic orbit solution, it is a sufficient condition for the existence of multi-pulse and periodic multi-pulse solutions.
...
We now address the existence of a primary pulse solution, which is a symmetric homoclinic orbit connecting the unstable manifold $\tilde{W}^u(0; c_0)$ and the stable manifold $\tilde{W}^s(0; c_0)$ of the rest state equilibrium. Both of these manifolds are dimension $m$ by reversibility. In general, the existence of such a solution is unknown, although in specific cases such as KdV5 we do have an existence result (\cite{Pelinovsky2007}). We will thus take the existence of a primary pulse solution for a specific wavespeed $c_0$ as a hypothesis.
...
\noi It follows from \cref{Qexistshyp} that $q(x; c_0)$ is a symmetric, exponentially localized traveling wave solution solution to \cref{genPDE}. We take the following hypothesis regarding the intersection of the stable and unstable manifolds in $H^{-1}(0; c_0)$.
...
Finally, as in \cite{Grillakis1987}, define the scalar $d(c) = \calE(q(x, c)) - \omega\calQ(q(x, c))$. By \cite[(2.21)]{Grillakis1987},
...
\noi From this point on, we will fix a speed $c \in (c_0 - \delta, c_0 + \delta)$ and suppress the dependence on $c$ for simplicity.
...
\end{figure}
The existence of these Krein bubbles is given in the following theorem, which also provides numerically verifiable estimates for their size and location. The result holds under a mild assumption on the signs of $M^c$ and $\tilde{M}^c$, which can be checked numerically.

\begin{theorem}\label{th:Kreinbubble}
Assume Hypotheses \ref{hyp:E}, \ref{hyp:H}, \ref{hyp:hypeq}, \ref{Qexistshyp}, and \ref{hyp:transverse}, and \ref{hyp:dccpos}. In addition, assume $M^c \tilde{M}^c < 0$.

Choose $m_0 = 1$, and let $r_*$ be as in \cref{2pulsebifurcation}. Let 
\begin{align*}
\lambda_* &= \sqrt{ \frac{-2a - R_1 }{M} } + \mathcal{O}(r) = \sqrt{ \frac{-2a}{M} } + \mathcal{O}(r) \\
X_* &= c \left( \frac{\pi i}{\lambda_*} - \frac{K}{M}\right),
\end{align*}
and define $T_0 > 0$ by 
\[
T_0 = -\frac{M^c \tilde{M}^c}{2 \pi M^2 c } \lambda_*^5 X_0^2
\]
Let $Q_2(x; s, r)$ be the family of periodic 2-pulse solutions parameterized by a parameter $s$, where $s \in [-2\sqrt{T_0}, 2 \sqrt{T_0}]$, so that the domain size $X$ is given by
\begin{equation}
X = X_* + \frac{2 c \pi s}{\lambda_*^2} = \mathcal{O}(s^2).
\end{equation}

Then, as $s$ is varied, there is a pair of eigenvalues located at 
\begin{align}\label{Kreineigs}
\lambda = \lambda_* - s i \pm \sqrt{ T_0 -  s^2} + \mathcal{O}(\cdots)
\end{align}
\begin{enumerate}[(i)]
\item For
\[
s = s_\pm = \pm \sqrt{T_0}\left( 1 + \mathcal{O}\left(\frac{1}{|\log r|} \right)\right).
\]
there is a double eigenvalue on the imaginary axis at $\lambda = \lambda_* - s i \pm + \mathcal{O}\left(\frac{1}{|\log r|} \right)$. This occurs when 
\[
X = X_* \pm \frac{2 c \pi}{\lambda_*^2}\sqrt{T_0} + \mathcal{O}(s^2)
\]
\item For $s \in (s_-, s+)$, equation \cref{Kreineigs} is a pair of eigenvalues which is symmetric across the imaginary axis. The real part of \cref{Kreineigs} attains its maximum of approximately $\sqrt{T_0}$ when $s = 0$
...
\begin{aligned}
\det S(&\mu, r) = -2 \mu^2 c^2 (2a + \mu^2 c^2 + R_1) \left( M \sinh(\mu X) + c \mu (M \tilde{M} - M^c \tilde{M}^c) \cosh(\mu X) \right) \\
...
\begin{aligned}
\det S(&\mu, r) = -2 c^2 \mu^2 (2a + c^2 \mu^2 M) \left( M \sinh(\mu X) + c \mu (M \tilde{M} - M^c \tilde{M}^c) \cosh(\mu X) \right) + \tilde{R} \mu^2 \sinh(\mu X) + R \mu^3,
\end{aligned}
\end{equation}
...
+ 2 M M^c \tilde{M}^c c \tilde{\mu}^5 r^{5/2} X_0^2 + \mathcal{O}( r^{5/2} X_0 ) = 0
\end{aligned}
\end{equation}
Since
\[
...
\begin{aligned}
2 \pi c M^2 X_0^2 r^{5/2} \tilde{h}( \tilde{h} + 2 \tilde{k} i) 
...
- T + \mathcal{O}\left( \frac{1}{|\log r|} \right) = 0, \quad T = -\frac{M^c \tilde{M}^c }{2 \pi M^2 } c^2 \tilde{\mu}^5.
\end{aligned}
\end{equation}
Since we are assuming $M > 0$ and $M^c \tilde{M}^c < 0$, $T > 0$. When $r = 0$, $G(\tilde{h}, \tilde{k}, 0) = 0$ if and only if 
\begin{equation}
...
\begin{equation}
\partial_{\tilde{h}_0}G(\tilde{h}_0^\pm(\tilde{k}), \tilde{k}, 0) = 2 \sqrt{T - \tilde{k}^2},
\end{equation}
...
\end{pmatrix},
\end{equation*}
which is nonsingular. Using the implicit function theorem, there exists $r_2 \leq r_1$ and a unique smooth function $(\tilde{h}_1(r), \tilde{k}_1(r))$ with $(\tilde{h}_1(0), \tilde{k}_1(0)) = (-\sqrt{T}i, \sqrt{T})$ such that for all $r \leq r_2$, $H(\tilde{h}_1(r), \tilde{k}_1(r), r) = 0$, i.e. $\tilde{h}_1(r)$ is a double root of $G(\tilde{h}, \tilde{k}, r)$ when $\tilde{k} = \tilde{k}(r)$. By Hamiltonian symmetry, $\tilde{h}(r)$ must lie on the imaginary axis. We have a similar result for $(\tilde{h}, \tilde{k}) = (\sqrt{T}i, -\sqrt{T})$.

Undoing the scaling and change of variables and letting $s = k/c$, for $r \leq r_2$ there is a pair of eigenvalues at
\begin{align*}
\lambda = \lambda_* - s i \pm \sqrt{ T_0 -  s^2}, \quad T_0 = -\frac{M^c \tilde{M}^c}{2 \pi M^2 c } \lambda_*^5 X_0^2,
\end{align*}
