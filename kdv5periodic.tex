...
P^+(x; \lambda) = I + \Theta^+(x; \Lambda)
\end{equation}
such that for all $|\lambda| < \delta_3$, the change of coordinates $V(x) = P^+(x; \lambda) Z^+(x)$ conjugates \cref{vareqlambda} into the constant-coefficient equation $(Z^+)'(x) = A(0; \lambda) Z^+(x)$. $\Theta^+(x; \lambda)$ has the uniform decay rate
...
Let $E^{u/s/c}(0)$ be the stable, unstable, and center eigenspaces of $A(0)$, and $P^{u/s/c}(0)$ be their respective eigenprojections. Let $E^{u/s/c}(\lambda)$ and $P^{u/s/c}(\lambda)$ be the corresponding eigenspaces and eigenprojections for $A(0; \lambda)$, which are smooth in $\lambda$. $E^s(\lambda)$ and $E^u(\lambda)$ are $m$-dimensional, and $E^c(\lambda)$ is 1-dimensional. In the next lemma, we prove some useful results about $A(0; \lambda)$.
...
\]
By Hamiltonian symmetry (and the symmetry of $E(\lambda)$, eigenvalues must come in quartets. Since there is nothing else above the real axis with similar magnitude, $\lambda_m^{\text{ess}}(r)$ is on the imaginary axis, and there is another essential spectrum eigenvalue at $-\lambda_m^{\text{ess}}(r)$. 
