...
\begin{aligned}
\det S(\lambda, r) &= \lambda^2 \Big( 2 M \sinh(\nu(\lambda)X)(1 + \mathcal{O}(|\lambda|^2 + r^{1/2})) + \lambda(M \tilde{M} - M^c \tilde{M}^c)\cosh(\nu(\lambda)X) \\
...
For the periodic single pulse, there is a single length parameter $X_0$, and for the ansatz \cref{Vpiecewise}, there is only one parameter $c$ and one parameter $d$. The system of equations \cref{eigsystem} becomes
...
W^+(0) - W^-(0) &+ c ( e^{-\nu(\lambda)X_0}V_0^+(0; \lambda) - e^{\nu(\lambda)X_0} V_0^-(0; \lambda)) \in \C \Psi(0) \oplus \C \Psi^c(0)\end{aligned}
\end{equation}
...
\end{align*}
thus $\det S(-\lambda) = \det K \det S(\lambda) = -\det S(\lambda)$, and we compute $\det S(\lambda)$ directly to get \cref{1pblockmatrixdet}.

\section{Proof of Theorem \ref{theorem:1pess} }
