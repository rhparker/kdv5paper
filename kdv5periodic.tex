...
\]
which is a weakly nonlinear long-wave approximation for capillary-gravity water waves \cite{Champneys1998,Champneys1997}. We will consider the simpler equation 
...
\end{equation}
where $\calE''(u^*)$ is the Hessian of the energy. The rest state $u = 0$ corresponds to the equilibrium point $U = 0$ of the first order system \cref{KdV5ham2}. When $p < 2 \sqrt{c}$, this equilibrium is a hyperbolic saddle with 2-dimensional stable and unstable manifolds. The primary pulse $q(x)$ corresponds to a homoclinic orbit connecting the stable and unstable manifolds of this equilibrium. If $-2 \sqrt{c} < p < 2 \sqrt{c}$, the eigenvalues of $DF(0)$ are a complex quartet $\pm \alpha_0 \pm \beta i_0$. For $-2 \sqrt{c} < p < 2 \sqrt{c}$, multi-modal homoclinic and periodic orbits exist which lie close to the primary homoclinic orbit. We will adapt Lin's method as in \cite{Sandstede1993, SandstedeStrut} to construct periodic multi-pulse solutions by splicing together consecutive copies of the primary pulse using small remainder functions. This provides not only an existence result but also estimates for these small remainder functions. As with $n$-homoclinic orbits, the distances between neighboring peaks are constrained \cite{SandstedeStrut,Sandstede1998}, but there is an additional degree of freedom in the periodic case. This leads to a pitchfork bifurcation structure for periodic double pulses. To examine the spectral stability of a periodic $n$-pulse $q_n$, we will study the spectrum of the linear operator $\partial_x \calE''(q_n)$, which is the linearization of the PDE \eqref{KdV5c} about $q_n$. As in \cite{Sandstede1998}, we will reduce the computation of the spectrum in a ball around the origin to computing the determinant of a matrix $S(\lambda)$. For a periodic $n$-pulse, $S(\lambda)$ is a $2n \times 2n$ matrix, and solving $\det S(\lambda) = 0$ will give us both the interaction eigenvalues and the essential spectrum eigenvalues. 
...
Linearizing the PDE \cref{genPDE} about a traveling wave solution $\phi_c$, we obtain the  linear operator $\partial_x \calL(\phi_c)$, where $\calL(\phi_c)$ is the self-adjoint operator
...
Let $U^*(x) = (u^*(x), \partial_x u^*(x), \dots, \partial_x^{2m-1}u^*(x) )^T$ be any solution to \cref{genODE}, so that $u^*(x)$ is a traveling wave solution to \cref{genPDE}. Then $u^*(x)$ also solves the equation $\partial_x(\calE'(u) + cu) = 0$, which is equivalent to the system
...
\end{pmatrix},
\end{equation}
where $e_{2m} = (0, \dots, 0, 1)^T \in \R^{2m}$ is the standard unit vector. Since $U^*(x)$ solves \cref{genODE}, $(U^*(x), 0)^T$ is a solution to \cref{spsystem2}. We similarly reformulate the PDE eigenvalue problem $\partial_x \calL(u^*) v = \lambda v$ as the system
...
The rest state $(U, k) = (0, 0)$ is a non-hyperbolic equilibrium of \cref{spsystem2}. $A(0)$ has a one-dimensional kernel, which is characterized in the following lemma. 
...
\]
for which $W_0 = (0, \dots, 0, 1)^T$ is a constant solution. Using \cref{psiform} below, $\Psi(x) = ( \nabla H(Q(x)), q(x) )^T$ is an exponentially localized solution. $\Psi(0)$ and $W_0$ are perpendicular to $\R Q'(0) \oplus Y^+ \oplus Y^-$ at $x = 0$ by \cref{eigadjoint} below. Since $\Psi(0)$ and $\Psi^c(0)$ are linearly independent, there can be no other bounded, linearly independent solutions to \eqref{adjvareq2}.
\end{proof}
\end{lemma}
...
We now look at the spectrum of the periodic multi-pulses which we constructed in the previous section. Let $Q(x)$ be the primary pulse solution, and let $Q_n(x) = (q_n(x), \partial_x q_n(x), \dots, \partial_x^{2m} q_n(x), 0)$ be any periodic $n$-pulse solution constructed according to \cref{th:perexist} on periodic domain $[-X, X]$. It is natural to pose the PDE eigenvalue problem \cref{genPDEeig2} on the space of periodic functions $H^{2m}_{\text{per}}[-X,X]$, where
\[
...
Following the spatial dynamics approach from \cref{sec:EVP}, equation \cref{genPDEeig2} on $H^{2m}_{\text{per}}[-X,X]$ is equivalent to the first order system with periodic boundary conditions
...
The simplest case is the periodic single pulse. Proofs of all results are given in \cref{sec:singlepulse}. There is a only single length parameter $X_0$, which is the same as the domain length $X$. The block matrix $S(\lambda)$ is a $2\times 2$ matrix, the form of which is given in the following lemma.
...
The next simplest case is the periodic double pulse. Proofs of all results are given in \cref{sec:doublepulse}. In this case, the block matrix $S(\lambda)$ is a $4\times 4$ matrix, the form of which is given in the next lemma.
...
Finally, we consider what happens when the first essential spectrum eigenvalue collides with an imaginary interaction eigenvalue. Choose $m_0 = 1$, so that the interaction eigenvalues will be purely imaginary by \cref{theorem:2peigsassym}, and let $Q_2(x; s_1, r)$ be the periodic double pulse constructed according to \cref{2pulsebifurcation}. The interaction eigenvalues scale like $r^{1/2}$, and the first essential spectrum eigenvalue scales like $1/X(r, s_1)$. For fixed $r$, there will be a collision between these two eigenvalues on the imaginary axis at a specific value of $s_1$. As $s_1$ is increased, the eigenvalues, collide, move off the imaginary axis, trace an approximate circle in the complex plane, and finally recombine on the imaginary axis in a ``reverse'' Krein collision, which is shown in \cref{fig:KreinBubbleCartoon}. The existence of the first Krein bubble is given in the following theorem, which also provides numerically verifiable estimates for its size and location. Results for subsequent Krein bubbles can be similarly obtained.
