...
\begin{enumerate}[(i)]
\item The change of coordinates $W = P^\pm Z$ reduces \eqref{EVPconj} to the equations on $\R^\pm$
...
Let $E^{u/s/c}(0)$ be the stable, unstable, and center eigenspaces of $A(0)$ and $P^{u/s/c}(\lambda)$ the corresponding eigenprojections. $E^s(0)$ and $E^u(0)$ are $m$-dimensional, and $E^c(0)$ is 1-dimensional. Choose $\delta \leq \min\{\delta_0, \delta_1\}$ so that for all $|\lambda| < \delta$, $|\nu(\lambda)| < \eta$, where $\nu(\lambda)$ defined in Lemma \ref{nulambdalemma}, and the real part of any other eigenvalue of $A(\lambda)$ lies strictly outside the interval $[-\alpha, \alpha]$. Let
\begin{equation}
...
\end{equation}
where $U(x) \in C_b([0, \infty), \R^{2m+1})$ or $U(x) \in C_b((-\infty, 0], \R^{2m+1})$, with $|U(x)| \leq C (e^{-\alpha_0 |x|})$. Let $\Lambda = (U(x), \lambda)$ and $\Lambda_0 = (Q(x), 0)$. Using the conjugation lemma, there exists $\delta_2 > 0$ with $0 < \delta_2 \leq \delta_1$ and invertible linear transformations
\[
P^\pm(x; U(x), \lambda) = I + \Theta(x; U(x), \lambda)
\]
such that for all $|\lambda| < \delta$ and $U(x)$ with $\| U(x) - Q(x) \| < \delta$, the change of change of coordinates $V = P^\pm(x; U(x), \lambda)$ on $\R^\pm$ conjugates \cref{Veqgeneral} into the into the constant-coefficient equation $Z'(x) = A(\lambda) Z(x)$. By Lemma \ref{solvewithjumps}, $\| Q_i^\pm(x) - Q(x) \| \leq C e^{-\alpha X_*}$, thus we can choose $X_*$ sufficiently large so that $\| Q_i^\pm(x) - Q(x) \| \leq \delta$ for $i = 0, \dots, n-1$. Define
\[
P_i^\pm(x; \lambda) = I + \Theta_i^\pm(x; \lambda) = I + \Theta(x; Q_i^\pm(x), \lambda)).
\]
Expanding in a Taylor series about $Q(x)$, and using \cref{conjthetadecay} and the estimates from Lemma \ref{solvewithjumps}, $\Theta_i^\pm(x; \lambda) = \Theta^\pm(x; \lambda) + \mathcal{O}(e^{-2 \alpha X^*})$, thus we have the estimates
...
We can now construct our piecewise ansatz using \cref{Qnppiece} and \cref{defVipm}. It follows from \cref{Ekernel} and \cref{defAB} that
...
\end{cases}
\end{equation}
for $i = 0, \dots, n-1$, where all subscripts are taken $\Mod n$, and the pieces are joined together end-to-end as in \cite{Sandstede1998}. For the remainder functions, $W_i^-(x) \in C([-X_{i-1}, 0], \C)$ and $W_i^+(x) \in C([0, X_i], \C)$, and $c_i, d_i \in \C$ are constants. The factors $e^{-\nu(\lambda)X_i}$ and $e^{\nu(\lambda)X_{i-1} }$ will facilitate joining the pieces at $\pm X_i$. Substituting \cref{Vpiecewise} into \cref{PDEeigsystemper3} and simplifying, we obtain the equation for $W_i^\pm(x)$
\begin{equation}\label{Wipm2}
\begin{aligned}
(W_i^\pm)'(x) &= A(Q_i^\pm(x); \lambda) W_i^\pm(x) + d_i \lambda^2 \tilde{H}_i^\pm(x) \\
\end{aligned}
\end{equation}
where
\begin{align}\label{deftildeH}
\tilde{H}_i^\pm(x) &= -B \partial_c Q_i^\pm(x), \quad H(x) = -B \partial_c Q(x)
\end{align}

Since the eigenfunction $V(x)$ must be continuous, the $2n$ pieces \cref{Vpiecewise} must  be joined together end-to-end in a loop, i.e. $n$ matching conditions at $x = \pm X_i$ and $n$ matching conditions at $x = 0$ must be satisfied. Thus to obtain an eigenfunction $V(x)$, we must satisfy the system of equations
...
C_i c &= c_i \left( e^{\nu(\lambda) X_i} V_i^-(-X_i; \lambda) - e^{-\nu(\lambda) X_i} V_i^+(X_i; \lambda) \right) \label{defCic}
\end{align}
...
(Z_i^\pm)'(x) = A(\lambda) Z_i^\pm(x) + \lambda^2 d_i P_i^\pm(x; \lambda)^{-1} \tilde{H}_i^-(x) \\
\end{equation}
