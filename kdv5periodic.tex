...
2 M \sinh(\mu X) + c K \mu \cosh(\mu X) + \mathcal{O}(|\mu|(|\mu|^2 + r^{1/2} X ) = 0
\end{equation}
...
\begin{align*}
\left( 2M + \frac{cK}{X} \right)h + \mathcal{O}\left( \frac{m+h}{X}\left(\frac{m+h}{X} + r^{1/2} \right) \right) = 0
\end{align*}
Since $X = X(r) = \mathcal{O}(|\log r|)$ and $|\log r|^{-1}$ is lower order than $r^{1/2}$, we wish to solve
\begin{align*}
...
\begin{aligned}
\det S(\mu, r) &= -2 c^2 \mu^2 (2a + c^2 \mu^2 M)\left( M \sinh(\mu X) + \lambda(M \tilde{M} - M_c \tilde{M}_c)\cosh(\mu X) \right) + \mathcal{O}\left((|\mu| + r^{1/2})^5 \right).
\end{aligned}
\end{equation}
Define
\begin{align}
...
\begin{align*}
\left( 2a + M c^2 \left(\frac{m \pi i +h}{X} \right)^2 \right) \left( M + \frac{cK}{X} \right)h
+ \mathcal{O}\left( \left(\frac{m+h}{X} \right)^3 \right) = 0,
\end{align*}
where we used the fact that $r^{1/2}$ is higher order than $\mu$. Since $a = \mathcal{O}(r)$ and $X = \mathcal{O}(|\log r|)$, multiply by $X^2$ and simplify to get
\begin{align}
G_m(h, r) = M c^2 (m \pi i + h)^2 h
+ \mathcal{O}\left( \frac{(m+h)^3}{|\log r|} \right) = 0.
\end{align}
Since $G_m(0,0) = 0$ and $\partial_h G_m(0,0) = -M c^2 m^2 \pi^2 \neq 0$, the result follows from the implicit function theorem and Hamiltonian symmetry as in the proof of \cref{theorem:1pess}.
...
We can now find the interaction eigenvalues. To rescale equation \cref{2pdetmu2}, let $a = r \tilde{a}$, where $\tilde{a} = \tilde{a}(r; m_0, s_1)$ is defined in \cref{lemma:chara} and $\tilde{a}(0; m_0, s_1) \neq 0$ since $s_1 > p^*$. Let $\mu^*_{int} = r^{1/2} \tilde{\mu}^*$, where $\tilde{\mu}^* \neq 0$, and take the ansatz $\mu = r^{1/2} \tilde{\mu}^* + r^{1/2}$. Substituting this into \cref{2pdetmu2}, dividing by $-2 c^2 r^{5/2} X$, and using the fact that $\mu = \mathcal{O}(r^{1/2})$ and $X = \mathcal{O}(|\log r|)$, we obtain the equation
\[
G(h, r) = M^2 c^2 h (\tilde{\mu}^* + h)^3 (2 \tilde{\mu}^* + h) + \mathcal{O}\left(\frac{1}{|\log r|} \right) = 0
\]
Since $G(0, 0) = 0$ and $\partial_h G(0, 0) = 2 c^2 M^2 (\tilde{\mu}^*)^4 \neq 0$, by the implicit function theorem there exists $r_1 \leq r_0$ and a unique smooth function $h(r)$ with $h(0) = 0$ such that for all $r \leq r_1$, $G(h(r), r) = 0$. 


*** can we do better on remainder terms? ***


to solve $G(\tilde{h},r) = 0$ for $\tilde{h}$ in terms of $r$ near $\tilde{h} = 0$. Specifically, there exists $r_4 \leq r_0$ and a unique smooth function $\tilde{h}(r)$ with $\tilde{h}(0) = 0$ such that for all $r \leq r_4$, $G(\tilde{h}, r) = 0$. Expanding $\tilde{h}(r)$ in a Taylor series about $r = 0$, for all $r \leq r_4$,
\[
