...
The simplest case is the periodic single pulse. Proofs of all results in this section are given in \cref{sec:singlepulse}. There is a only single length parameter $X_0$, which is the same as the domain length $X$. The block matrix $S(\lambda)$ is a $2\times 2$ matrix, the form of which is given in the following lemma.
...
The next simplest case is the periodic double pulse. Proofs of all results in this section are given in \cref{sec:doublepulse}. In this case, the block matrix $S(\lambda)$ is a $4\times 4$ matrix, the form of which is given in the next lemma.
