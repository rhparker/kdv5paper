...
\end{align*}
from which we obtain the form of the remainder matrix $R(\lambda)$. It follows that $\det S(-\lambda) = \det K_1 \det S(\lambda) \det K_2 = -\det S(\lambda)$. The determinant of $S(\lambda)$ is computed directly with the aid of Mathematica, where we use the odd symmetry of $S(\lambda)$ the fact that $\lambda = 0$ is an eigenvalue with at least algebraic multiplicity 3.
