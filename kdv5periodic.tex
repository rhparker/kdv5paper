...
\end{equation}
where $R$ is standard reversor operator. Let $\alpha_0, \beta_0$ be defined as in \cref{hyp:hypeq}. Choose any $\eta > 0$ with $4 \eta < \alpha_0$, and let $\alpha = \alpha_0 - 2 \eta$. Let $\delta_1$ be as in \cref{nulambdalemma}, and choose $\delta_2 < \delta_1$ sufficiently small so that for all $|\lambda| < \delta_2$,
\begin{enumerate}[(i)]
...
\end{equation}
For all $\lambda$, $|A(Q(x); \lambda) - A(\lambda)| \leq C e^{-\alpha_0 |x|}$, i.e $A(Q(x); \lambda)$ decays exponentially to the constant-coefficient matrix $A(\lambda)$. Using the conjugation lemma on $\R^+$ with $\Lambda = \lambda$ and $\Lambda_0 = 0$, there exists $\delta_3 < \delta_2$ and an invertible linear transformation 
...
\end{align}
where $|V_1^\pm(x; \lambda)| \leq C e^{-\alpha |x|}$ and $V^-(x; \lambda) = R V^+(-x; -\lambda)$.
\begin{proof}
...
\end{aligned}
\end{equation}
which are the same as the estimates \cref{stdtrichbounds} but with possibly different constants out front. The evolution of the unconjugated system $W_i^\pm(x) = A(Q_i^\pm(x); \lambda) W_i^\pm(x)$ on $\R^\pm$ is given by
